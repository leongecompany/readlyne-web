...
function getAdminToken(): string {
  const stored = typeof window !== 'undefined' ? localStorage.getItem('readlyne_admin_token') : null;
  return stored || encodeURIComponent(ACCESS_CODE);
}
...
export default function AdminDashboard() {
  ...
  const handleLogin = () => {
    if (code === ACCESS_CODE) {
      setAuthed(true);
      localStorage.setItem('readlyne_admin_token', encodeURIComponent(ACCESS_CODE));
  ...
  if (!authed) {
    return (
      <div style={{ maxWidth: 400, margin: '80px auto', padding: 20 }}>
        ...
        />
        <button className="btn-primary" onClick={handleLogin} style={{ fontSize: 16, width: '100%' }}>
          进入
        </button>
      </div>
    );
    ...
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '20px 16px 80px' }}>
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 20, overflowX: 'auto', paddingBottom: 4 }}>
        {[
          ...
          <button
            key={t.key}
            onClick={() => { setTab(t.key as Tab); queriesOffset === 0 ? {} : setQueriesOffset(0); }}
            style={{
        ...
        <div>
          ...
          <Section title="累计数据">
            <div style={{ display: 'flex', gap: 10 }}>
              <StatCard label="总请求" value={stats.total.total_all_time} />
              <StatCard label="总用户" value={stats.total.total_users_all_time} />
            </div>
          </Section>
      ...
      {tab === 'queries' && (
        <div>
          <div style={{ display: 'flex', gap: 8, marginBottom: 12, alignItems: 'center' }}>
            ...
            <span style={{ fontSize: 11, color: 'var(--text-tertiary)', marginLeft: 8 }}>共 {queriesTotal} 条</span>
          </div>
          ...
          {queriesTotal > 50 && (
            <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginTop: 16 }}>
              ...
              <button
                className="btn-secondary"
                disabled={queriesOffset + 50 >= queriesTotal}
